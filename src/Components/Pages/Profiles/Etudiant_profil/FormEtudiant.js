...
export default function FormEtudiant() {
  ...
  const [diplomeValue, setDiplomeValue] = useState("");
  const [filières, setFilière] = useState([""]);
  const [filièreValue, setFilièreValue] = useState("");
  console.log(filièreValue);
  const [diplomeCherche, setdiplomeCherche] = useState("");
  const [domaineDip, setdomaineDip] = useState([""]);
  const [domaineDipValue, setdomaineDipValue] = useState("");
  ...
  useEffect(() => {
    ...
    if (filièreValue === "Diplome classes préparatoires en arabe") {
      setSection([
        "Lettre",
        "Mathématique",
        "Science exprémentale",
    ...
    if (filièreValue === "Diplome classes préparatoires en anglais") {
      setSection([
        "Lettre",
        "Mathématique",
        "Science exprémentale",
    ...
    if (filièreValue === "Diplome classes préparatoires en français") {
      setSection([
        "Lettre",
        "Mathématique",
        "Science exprémentale",
 ...
 *   /**************************************************************************** */
  ...
  if (niveauEtude === "" && etudData.dataNiveau.Niveau === "Bachelier") {
      const dataBachelier = {
      ...
      axios({
        url: "http://localhost:4000/api/user/updateProfilEtudiant",
        method: "put",
        data: data,
      })
      .then((res)=>{
    ...
    if (niveauEtude === "" && etudData.dataNiveau.Niveau === "Diplômé") {
      const dataDiplome = {
        Niveau: props.niveauEtude,
  ...
  useEffect(() => {
    const getEtudData = async () => {
      await axios({
        ...
        setDomaine(res.data.dataNiveau.domaine);
        setDiplomeValue(res.data.dataNiveau.diplome);
        setFilièreValue(res.data.dataNiveau.filière);
        setSectionValue(res.data.dataNiveau.section);
        setdiplomeCherche(res.data.dataNiveau.diplomeDip);
        setdomaineDipValue(res.data.dataNiveau.domaineDip);
      });
    };

    getEtudData();
  }, [etud._id]); //UNE SEUL FOIS

  const initialValues = {
    img: etudData.img,
    nom: etudData.Nom_prénom,
    niveauEtude: etudData.dataNiveau.Niveau,
    ville: etudData.Ville,
    section: etudData.dataNiveau.section,
    diplome: etudData.dataNiveau.diplome,
    domaine: etudData.dataNiveau.domaine,
    filière: etudData.dataNiveau.filière,
    diplomeCherche: etudData.dataNiveau.diplomeDip,
    score: etudData.dataNiveau.score,
    specialite: etudData.dataNiveau.specialite,
    domaineDip: etudData.dataNiveau.domaineDip,
    accept: true,
  };
        ...
        {({ errors, touched, values }) => (
          <Form noValidate autoComplete="off">
            <div className={classes.photo}>
              <img
                src={preview ? preview : etudData.img}
                alt=""
            ...
            <Container style={{ marginTop: "20px" }}>
              ...
              <center>
                ...
                {/* *********************************************************************************** */}
                {niveauEtude === "" ? (
                  etudData.dataNiveau.Niveau === "Bachelier" ? (
                    <>
                      <Divider
                      ...
                      ) : etudData.dataNiveau.Niveau === "Diplômé" ? (
                        <>
                          <Divider
                            style={{ marginTop: "20px", marginBottom: "20px" }}
                          />
