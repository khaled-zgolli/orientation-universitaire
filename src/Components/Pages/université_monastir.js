...
export default function Universite_monastir(props) {
  ...
  useEffect(() => {
    const getConsdData = async () => {
      let search = "";
      if (props.location.pathname === "/université/monastir") {
        search = "Monastir";
      }
      if (props.location.pathname === "/université/sousse") {
        search = "Sousse";
      }
      if (props.location.pathname === "/université/manar") {
        search = "Tunis Manar";
      }
      if (props.location.pathname === "/université/zaytouna") {
        search = "Zaytouna";
      }
      if (props.location.pathname === "/université/carthage") {
        search = "Carthage";
      }
      if (props.location.pathname === "/université/tunis") {
        search = "Tunis";
      }
      if (props.location.pathname === "/université/manouba") {
        search = "Mnouba";
      }
      let aa = [];
      await axios({
        url: "http://localhost:4000/api/Affiche/ChercheEtablissementUniversite",
        method: "post",
        data: { universite: search },
      }).then(res => {
        setDatas(res.data);
        res.data.map(val => {
